...
print(f">> Gerando Raster MDS Veloz + Suavizado (Gauss): {f_name} | Res: {res}m")

# Carga seletiva ultra-rápida
...
# ==============================================================================
print(f"   - Interpolando {len(df)} pontos com Nearest (Velocidade Máxima)...")
interp = NearestNDInterpolator(np.column_stack((df.X, df.Y)), df.Z)
m_mds = interp(gx, gy)

# ==============================================================================
# PASSO 2: SUAVIZAÇÃO MATRICIAL (FILTRO GAUSSIANO)
# sigma=1.0 define o raio de suavização (cerca de 1 a 2 pixels de transição).
